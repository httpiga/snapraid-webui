...
test.describe("navigation and API smoke", () => {
  test("disks page loads with heading and tabs", async ({ page }) => {
    ...
    await expect(
      page.getByRole("tab", { name: "Visual Editor" }),
    ).toBeVisible()
  })

  test("operations page loads with heading and command selection", async ({ page }) => {
  ...
  test("schedules page loads with heading and new schedule action", async ({ page }) => {
  ...
  test("recovery page loads with heading and main content", async ({ page }) => {
  ...
  test("logs page loads with heading and list or empty state", async ({ page }) => {
    await page.goto("/logs")
    await expect(
      page.getByRole("heading", { name: "Logs" }),
    ).toBeVisible()
    await expect(page.getByText(/log files? found/i)).toBeVisible()
  })
